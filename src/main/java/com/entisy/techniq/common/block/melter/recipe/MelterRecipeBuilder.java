...
public class MelterRecipeBuilder {
    ...
    public MelterRecipeBuilder(IItemProvider provider, int count) {
        this.result = provider.asItem();
        this.count = count;
    }

    public static MelterRecipeBuilder melter(IItemProvider provider) {
        return new MelterRecipeBuilder(provider, 1);
    }

    public static MelterRecipeBuilder melter(IItemProvider provider, int count) {
        return new MelterRecipeBuilder(provider, count);
    }

    public MelterRecipeBuilder requiredEnergy(int requiredEnergy) {
        this.requiredEnergy = requiredEnergy;
        return this;
    }

    public MelterRecipeBuilder smeltTime(int smeltTime) {
        this.smeltTime = smeltTime;
        return this;
    }

    public MelterRecipeBuilder requires(ITag<Item> tag) {
        return this.requires(Ingredient.of(tag));
    }

    public MelterRecipeBuilder requires(IItemProvider provider) {
        return this.requires(provider, 1);
    }

    public MelterRecipeBuilder requires(IItemProvider provider, int count) {
    ...
    public MelterRecipeBuilder requires(Ingredient ingredient, int count) {
        for (int i = 0; i < count; ++i) {
    ...
    public MelterRecipeBuilder unlockedBy(String p_200483_1_, ICriterionInstance p_200483_2_) {
        this.advancement.addCriterion(p_200483_1_, p_200483_2_);
        return this;
    }

    public MelterRecipeBuilder group(String p_200490_1_) {
