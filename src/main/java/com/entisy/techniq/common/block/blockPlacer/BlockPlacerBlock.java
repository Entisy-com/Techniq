...
public class BlockPlacerBlock extends ComplexMachineBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntityTypes.BLOCK_PLACER_TILE_ENTITY_TYPE.get().create();
    }
    ...
    public void setPlacedBy(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack item) {
        ...
        if (!item.getDisplayName().getString().isEmpty()) {
            ...
            if (tileEntity instanceof BlockPlacerTileEntity) {
                ((BlockPlacerTileEntity) tileEntity).setCustomName(item.getDisplayName());
            }
        }
    }
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        TileEntity tileEntity = world.getBlockEntity(pos);
        if (tileEntity instanceof BlockPlacerTileEntity && state.getBlock() != newState.getBlock()) {
            BlockPlacerTileEntity furnace = (BlockPlacerTileEntity) tileEntity;
