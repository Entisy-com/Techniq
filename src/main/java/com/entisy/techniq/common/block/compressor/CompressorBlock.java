...
public class CompressorBlock extends SemiComplexMachineBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntityTypes.COMPRESSOR_TILE_ENTITY.get().create();
    }
    ...
    public void setPlacedBy(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack item) {
        ...
        if (!item.getDisplayName().getString().isEmpty()) {
            ...
            if (tileEntity instanceof CompressorTileEntity) {
                ((CompressorTileEntity) tileEntity).setCustomName(item.getDisplayName());
            }
        }
    }
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        TileEntity tileEntity = world.getBlockEntity(pos);
        if (tileEntity instanceof CompressorTileEntity && state.getBlock() != newState.getBlock()) {
            CompressorTileEntity furnace = (CompressorTileEntity) tileEntity;
